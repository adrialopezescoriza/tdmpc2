...
def load_bc_results(tasks_demos):
    ...
    for task, demos in tasks_demos.items():
        ...
        for _, row in last_df.iterrows():
            bc_results[task].append(row['success'])
    return bc_results
...
def compute_domain_averages():
    ...
    for domain, tasks_demos in TASKS_DEMOS.items():
        ...
        # 🆕 Load BC results per task
        domain_bc_results[domain] = load_bc_results(tasks_demos)
    ...
    for domain, algo_results in domain_results.items():
        ...
        # 🆕 Compute BC average
        bc_values = []
        for values in domain_bc_results[domain].values():
            bc_values.extend(values)
        domain_averages[domain]["BC"] = sum(bc_values) / len(bc_values) if bc_values else 0
...
def plot_barplot(domain_averages):
    ...
    for i, domain in enumerate(domains):
        ...
        # 🆕 Assign labels and colors, including BC
        df['Algorithm Label'] = df['Algorithm'].map(lambda x: ALGO_TO_LABEL.get(x, x))
        palette = {
            algo: COLORS[ALGO_TO_COLOR[algo]] for algo in df['Algorithm'] if algo in ALGO_TO_COLOR
        }
        palette['BC'] = 'grey'
        ...
        sns.barplot(
            ...
            ax=ax, 
            palette=[palette[algo] for algo in df['Algorithm']],
            width=0.9,
            legend=False,
        )
    ...
    handles = [
        Line2D([0], [0], color=COLORS[ALGO_TO_COLOR[algo]], lw=6)
        for algo in ALGORITHMS
    ]
    labels = [ALGO_TO_LABEL[algo] for algo in ALGORITHMS]

    # Add BC manually
    handles.append(Line2D([0], [0], color='grey', lw=8))
    labels.append("BC")
