...
class TDMPC2(torch.nn.Module):
	...
	def _plan(self, obs, t0=False, eval_mode=False, task=None):
		...
		std = torch.full((b_size, self.cfg.horizon, self.cfg.action_dim), self.cfg.max_std, dtype=torch.float, device=self.device)
		if not t0:
		...
		for _ in range(self.cfg.iterations):
			...
			# Sample actions
			r = torch.randn(b_size, self.cfg.horizon, self.cfg.num_samples-self.cfg.num_pi_trajs, self.cfg.action_dim, device=std.device)
			...
			mean = (score.unsqueeze(1) * elite_actions).sum(dim=2) / (score.sum(1, keepdim=True) + 1e-9)
			std = ((score.unsqueeze(1) * (elite_actions - mean.unsqueeze(2)) ** 2).sum(dim=2) / (score.sum(1, keepdim=True) + 1e-9)).sqrt()
		...
		rand_idx = math.gumbel_softmax_sample(score.squeeze(-1), dim=1)  # gumbel_softmax_sample is compatible with cuda graphs
		actions = torch.stack([elite_actions[i,:,rand_idx[i]] for i in range(rand_idx.shape[0])], dim=0)
	...
	def _update(self, obs, action, reward, task=None, modify_reward = None, action_penalty = False):
		# Compute targets
		with torch.no_grad():
			# Get latent states
